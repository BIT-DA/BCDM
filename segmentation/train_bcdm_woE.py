...
def train(cfg, local_rank, distributed):
    ...
    for i, ((src_input, src_label, src_name), (tgt_input, _, _)) in enumerate(zip(src_train_loader, tgt_train_loader)):
        ...
        # Step B: train Classifiers to maximize loss_cdd
        optimizer_fea.zero_grad()
        optimizer_cls.zero_grad()
        ...
        loss_cdd = - cfg.SOLVER.CDD_LOSS * discrepancy_calc(tgt_pred, tgt_pred_2)
        loss_cdd.backward()
        optimizer_cls.step()
        ...
        # tgt_pred_2 = tgt_pred_2.div(temperature)
        ...
        #                                                 torch.cat((tgt_soft_label, torch.zeros_like(tgt_soft_label)),
        #                                                           dim=1))
        ...
        if (iteration == cfg.SOLVER.MAX_ITER or iteration % cfg.SOLVER.CHECKPOINT_PERIOD == 0) and save_to_disk:
            ...
            # run_test(cfg, feature_extractor, classifier, classifier_2, local_rank, distributed)
