...
def train(cfg, local_rank, distributed, cos_weight):
    ...
    optimizer_cls = torch.optim.SGD(list(classifier.parameters()) + list(classifier_2.parameters()), lr=cfg.SOLVER.BASE_LR*10, momentum=cfg.SOLVER.MOMENTUM, weight_decay=cfg.SOLVER.WEIGHT_DECAY)
    ...
    for i, (src_input, src_label, _) in enumerate(train_loader):
        ...
        pred_2 = classifier_2(feature_extractor(src_input), size)

        loss = criterion(pred, src_label) + criterion(pred_2, src_label)
        loss.backward()
        ...
        if cos_weight:
            ...
            if backbone_name == 'resnet101':
                ...
                for (w5, w6) in zip(classifier.parameters(), classifier_2.parameters()):
            ...
            Lambda_weight = 0.01
            damping = (1 - iteration / cfg.SOLVER.STOP_ITER)
...
def main():
    ...
    parser.add_argument(
        "--cos_weight",
        help="Whether use CLAN cos discrepancy loss when pre-training",
        action="store_true",
    )
    parser.add_argument(
        "--skip-test",
    ...
    model = train(cfg, args.local_rank, args.distributed, args.cos_weight)
