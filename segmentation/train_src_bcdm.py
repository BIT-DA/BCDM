...
def train(cfg, local_rank, distributed):
    ...
    if cfg.resume:
        ...
        classifier_2_weights = checkpoint['classifier_2'] if distributed else strip_prefix_if_present(
...
def run_test(cfg, model, local_rank, distributed):
    ...
    feature_extractor, classifier, classifier_2 = model
    ...
    with torch.no_grad():
        for i, (x, y, _) in enumerate(test_loader):
            ...
            pred_2 = classifier_2(feature_extractor(x))
            pred = F.interpolate(pred + pred_2, size=size, mode='bilinear', align_corners=True)
